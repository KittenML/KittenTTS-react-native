...
- [Bundled Offline Assets](#bundled-offline-assets)
...
In Expo apps, add the KittenTTS config plugin so prebuild includes the generated
asset directory in the native app bundle:
...
If the app already has generated `ios/` or `android/` folders from before the
...
| [`examples/OfflineBundledAssetsExample`](examples/OfflineBundledAssetsExample) | Bundled model and phonemizer wiring | Reference app code |
